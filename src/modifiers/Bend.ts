...
export class Bend extends Modifier implements IModifier {
	...
	constructor(f: number = 0, o: number = .5, a: number = 0) {
		super();
		this._force = f;
		this._offset = o;
		this.angle = a;
	...
	public set force(f: number) { this._force = f; }
	public get force(): number { return this._force; }

	public set constraint(c: number) { this._constraint = c; }
	public get constraint(): number { return this._constraint; }

	public get offset(): number { return this._offset; }
	public set offset(offset: number) { this._offset = offset; }

	public get diagAngle(): number { return this._diagAngle; }
	...
	public apply(): void {
		...
		for (let i: number = 0; i < vc; i++) {
			...
			if ((this.constraint == ModConstant.LEFT && p <= this.offset) || (this.constraint == ModConstant.RIGHT && p >= this.offset)) {
			} else {
